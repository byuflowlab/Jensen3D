...
def loss(r_0, a, alpha, x_focus, x, overlap):
    ...
    for i in range(0, np.size(x)):
        dx[i] = x_focus-x[i]
        if dx[i] > 0:
            loss[i] = overlap[i]*2.*a*(r_0[i]/(r_0[i]+alpha*(dx[i])))**2
...
def jensen_power(x, y, z, r_0, alpha, a, U_velocity, rho, Cp):
    ...
    for i in range(0, np.size(x)):
        A = r_0[i]**2*pi
        overlap_fraction = overlap(x, x[i], y, y[i], z, z[i], r_0, r_0[i], alpha)
...
if __name__ == '__main__':
    ...
    print Jensen_Wake_Model(x, y, z, params)
    ax = Axes3D(plt.gcf())
